...
class Database {
    ...
    public function __construct() {
        try {
            // Connessione database: SQLite per sviluppo, MySQL per produzione
            if (defined('DB_TYPE') && DB_TYPE === 'sqlite') {
                ...
                // Abilita chiavi esterne per SQLite
                $this->pdo->exec('PRAGMA foreign_keys = ON');
            } else {
                // Connessione MySQL per hosting Ionos
                $dsn = "mysql:host=" . DB_HOST . ";dbname=" . DB_NAME . ";charset=utf8mb4";
                $this->pdo = new PDO($dsn, DB_USERNAME, DB_PASSWORD);
                $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $this->pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);

                // MySQL ha chiavi esterne abilitate di default
            }

            // Inizializza database se necessario (solo per MySQL)
            if (!defined('DB_TYPE') || DB_TYPE !== 'sqlite') {
                $this->initDatabase();
            }
        ...
        try {
            ...
            try {
                ...
                if (strpos($e->getMessage(), 'no such column') !== false) {
                    $this->pdo->exec("ALTER TABLE provinces ADD COLUMN image_path VARCHAR(500) DEFAULT NULL");
                }
            }
            ...
            $this->pdo->exec("
                CREATE TABLE IF NOT EXISTS articles (
                    ...
                    city_id INTEGER,
                    author VARCHAR(500),
                    status VARCHAR(500) DEFAULT 'published',
            ...
            try {
                ...
                if (strpos($e->getMessage(), 'no such column') !== false) {
                    $this->pdo->exec("ALTER TABLE articles ADD COLUMN seo_title VARCHAR(500) DEFAULT NULL");
                    $this->pdo->exec("ALTER TABLE articles ADD COLUMN seo_description VARCHAR(500) DEFAULT NULL");
                    $this->pdo->exec("ALTER TABLE articles ADD COLUMN seo_keywords VARCHAR(500) DEFAULT NULL");
                }
            }
            ...
            $this->pdo->exec("
                CREATE TABLE IF NOT EXISTS businesses (
                    ...
                    longitude REAL,
                    status VARCHAR(500) DEFAULT 'pending',
                    subscription_type VARCHAR(500) DEFAULT 'free',
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            ...
            $this->pdo->exec("
                CREATE TABLE IF NOT EXISTS events (
                    ...
                    longitude REAL,
                    status VARCHAR(500) DEFAULT 'pending',
                    source VARCHAR(500) DEFAULT 'admin',
                    business_id INTEGER,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (category_id) REFERENCES categories (id) ON DELETE SET NULL,
                    FOREIGN KEY (province_id) REFERENCES provinces (id) ON DELETE SET NULL,
            ...
            $this->pdo->exec("
                CREATE TABLE IF NOT EXISTS place_suggestions (
                    ...
                    latitude REAL,
                    longitude REAL,
                    suggested_by_name VARCHAR(500) NOT NULL,
                    suggested_by_email VARCHAR(500) NOT NULL,
                    images VARCHAR(500),
                    status VARCHAR(500) DEFAULT 'pending',
                    admin_notes VARCHAR(500),
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            ...
            $this->pdo->exec("
                CREATE TABLE IF NOT EXISTS province_gallery (
                    ...
                    uploaded_by VARCHAR(500) DEFAULT 'admin',
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (province_id) REFERENCES provinces (id) ON DELETE CASCADE
                )
            ");
            ...
            $this->pdo->exec("
                CREATE TABLE IF NOT EXISTS category_fields (
                    ...
                    field_type VARCHAR(500) NOT NULL DEFAULT 'text',
                    field_options VARCHAR(500),
