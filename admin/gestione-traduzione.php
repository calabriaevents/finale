...
$languages = [
    ...
    'pt' => ['name' => 'Português', 'flag' => '🇵🇹', 'color' => 'teal'],
    'ru' => ['name' => 'Русский', 'flag' => '🇷🇺', 'color' => 'indigo'],
    'zh' => ['name' => '中文', 'flag' => '🇨🇳', 'color' => 'rose'],
    'ja' => ['name' => '日本語', 'flag' => '🇯🇵', 'color' => 'pink'],
    'ar' => ['name' => 'العربية', 'flag' => '🇸🇦', 'color' => 'cyan'],
    'nl' => ['name' => 'Nederlands', 'flag' => '🇳🇱', 'color' => 'orange'],
    'pl' => ['name' => 'Polski', 'flag' => '🇵🇱', 'color' => 'lime']
];
...
function countTranslatedFiles($language) {
    ...
    if ($language === 'it') {
        // Italiano: conta i file originali esistenti
        $count = 0;
    ...
    // Altre lingue: conta i file nella cartella specifica
    $count = 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'create_new_language':
            ...
            if (empty($new_language) || strlen($new_language) !== 2 || !ctype_alpha($new_language)) {
                $error = "❌ Codice lingua non valido (deve essere 2 lettere, es: 'pt')";
                break;
            }
            
            if (isset($languages[$new_language])) {
                $error = "❌ Lingua già esistente nel sistema";
                break;
            }
            
            if (empty($language_name) || empty($language_flag)) {
                $error = "❌ Nome lingua e bandiera sono obbligatori";
                break;
            }
            
            // Crea cartella lingua
            $language_dir = "../{$new_language}/";
            if (!is_dir($language_dir)) {
                if (!mkdir($language_dir, 0755, true)) {
                    $error = "❌ Impossibile creare la cartella /{$new_language}/";
                    break;
                }
            }
            
            // Crea cartella includes
            $includes_dir = "{$language_dir}includes/";
            if (!is_dir($includes_dir)) {
                if (!mkdir($includes_dir, 0755, true)) {
                    $error = "❌ Impossibile creare la cartella /{$new_language}/includes/";
                    break;
                }
            }
            
            // Copia file header e footer da 'en' (template base)
            $template_header = "../en/includes/header.php";
            $template_footer = "../en/includes/footer.php";
            $new_header = "{$includes_dir}header.php";
            $new_footer = "{$includes_dir}footer.php";
            
            if (file_exists($template_header)) {
                copy($template_header, $new_header);
            }
            if (file_exists($template_footer)) {
                copy($template_footer, $new_footer);
            }
            
            // Crea file index.php base
            $index_content = "<?php
// Homepage {$language_name} - {$language_flag}
// Creata automaticamente dal sistema di gestione traduzioni
...
\$featuredArticles = \$contentManager->getFeaturedArticles(6);
\$categories = \$contentManager->getCategories();
\$provinces = \$contentManager->getProvinces();
?>
<!DOCTYPE html>
<html lang=\"$new_language\">
<head>
    <meta charset=\"UTF-8\">
    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">
    <title>Passione Calabria - $language_name</title>
    <meta name=\"description\" content=\"Discover authentic Calabria\">
...
<body>
    ...
    <!-- Hero Section -->
    <section class=\"relative min-h-screen bg-gradient-to-br from-blue-600 via-purple-600 to-orange-500 flex items-center justify-center text-white\">
        <div class=\"absolute inset-0 bg-black bg-opacity-30\"></div>
        <div class=\"relative z-10 text-center max-w-4xl mx-auto px-4\">
            <h1 class=\"text-5xl md:text-7xl font-bold mb-6 leading-tight\">
                Discover <span class=\"text-yellow-300\">Calabria</span>
            </h1>
            <p class=\"text-xl md:text-2xl mb-8 leading-relaxed opacity-90\">
                Authentic beauty, crystal clear sea and thousand-year history
            </p>
            <div class=\"flex flex-col sm:flex-row gap-4 justify-center\">
                <a href=\"#explore\" class=\"bg-yellow-500 hover:bg-yellow-400 text-black px-8 py-4 rounded-full font-semibold text-lg transition-colors\">
                    🌊 Start Exploring
                </a>
                <a href=\"map.php\" class=\"bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-8 py-4 rounded-full font-semibold text-lg transition-colors backdrop-blur-sm\">
                    🗺️ View Map
                </a>
            </div>
        </div>
    ...
    <!-- NOTA: Questa è una pagina base. Traduci tutti i contenuti in $language_name -->
    <!-- TODO: Tradurre categorie, province e contenuti dinamici -->
    ...
    <script>
        lucide.createIcons();
            ...
            file_put_contents("{$language_dir}index.php", $index_content);
            
            // Aggiorna file di configurazione lingue (se esiste)
            $languages_config_file = "../includes/languages.php";
            if (!file_exists($languages_config_file)) {
                $languages_config_content = "<?php
// Configurazione lingue supportate
// File generato automaticamente

\$supported_languages = [];
";
                file_put_contents($languages_config_file, $languages_config_content);
            }
            
            $success_files = 3; // index.php, header.php, footer.php
            $message = "✅ Nuova lingua '{$language_name}' ({$new_language}) creata con successo!<br>📁 Cartella: /{$new_language}/<br>📄 File creati: {$success_files} (index.php, header.php, footer.php)<br>🎯 Prossimi passi: Traduci i contenuti nella nuova cartella";
            break;
...
<body class="min-h-screen bg-gray-50">
    ...
    <script>
        ...
        // Funzione per riempire i campi lingua con i suggerimenti
        function fillLanguage(code, name, flag) {
            document.getElementById('new_language').value = code;
            document.getElementById('language_name').value = name;
            document.getElementById('language_flag').value = flag;
        }
