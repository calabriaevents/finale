...
<!-- Basic Info -->
<div class="mb-4">
    <label for="title" class="block text-gray-700 font-bold mb-2">Titolo dell'articolo (es. Hotel Miramare - Tropea)</label>
...
<!-- Custom Fields for Accommodations -->
<div class="mt-6 p-6 bg-blue-50 rounded-lg border border-blue-200">
    <h3 class="text-lg font-semibold mb-4 text-blue-800">🏨 Dettagli Struttura Ricettiva</h3>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="mb-4">
            <label for="activity_name" class="block text-gray-700 font-bold mb-2">Nome della Struttura</label>
            <input type="text" name="json_data[activity_name]" id="activity_name" class="w-full px-3 py-2 border rounded-lg" value="<?php echo htmlspecialchars($json_data['activity_name'] ?? ''); ?>">
        </div>
        <div class="mb-4">
            <label for="average_price" class="block text-gray-700 font-bold mb-2">Fascia di Prezzo Media</label>
            <input type="text" name="json_data[average_price]" id="average_price" class="w-full px-3 py-2 border rounded-lg" placeholder="Es. €€ - €€€" value="<?php echo htmlspecialchars($json_data['average_price'] ?? ''); ?>">
        </div>
    </div>
</div>
...
<div class="mt-6 p-6 bg-gray-50 rounded-lg border">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div>
            <label for="city_autocomplete" class="block text-gray-700 font-bold mb-2">Città di Riferimento</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
            <label for="contact_website" class="block text-gray-700 font-bold mb-2">Sito Web</label>
            <input type="url" name="json_data[contact_details][website]" id="contact_website" class="w-full px-3 py-2 border rounded-lg" value="<?php echo htmlspecialchars($json_data['contact_details']['website'] ?? ''); ?>">
        </div>
    </div>
</div>

<!-- Description and Rooms -->
<div class="mt-6 p-6 bg-gray-50 rounded-lg border">
    <h3 class="text-lg font-semibold mb-4 text-gray-800">📝 Descrizione e Camere</h3>
    <div class="mb-4">
        <label for="content" class="block text-gray-700 font-bold mb-2">Descrizione (storia e unicità)</label>
    ...
    <div class="mb-4">
        <label for="room_details" class="block text-gray-700 font-bold mb-2">Tipologie di Camere e Prezzi</label>
        <textarea name="json_data[room_details]" id="room_details" rows="6" class="w-full px-3 py-2 border rounded-lg font-mono text-sm"><?php echo htmlspecialchars($json_data['room_details'] ?? ''); ?></textarea>
        <p class="text-xs text-gray-500 mt-1">Inserisci una camera per riga. Formato: <strong>Nome Camera: Prezzo: Descrizione</strong><br>Esempio: <strong>Suite Familiare: 150€: Due camere, vista mare, ideale per 4 persone.</strong></p>
    </div>
</div>
...
<div class="mt-6 p-6 bg-gray-50 rounded-lg border">
    ...
    <?php
        $predefined_services = ['Wi-Fi', 'Parcheggio', 'Aria Condizionata', 'Piscina', 'Centro Fitness', 'Navetta Aeroportuale'];
    ...
    <div>
        ...
        <input type="text" name="json_data[services][custom]" id="custom_services" class="w-full px-3 py-2 border rounded-lg" placeholder="Es. Animali ammessi, Accesso disabili, ..." value="<?php echo htmlspecialchars($current_services['custom'] ?? ''); ?>">
    </div>
</div>

<!-- Images -->
<div class="mt-6 p-6 bg-gray-50 rounded-lg border">
    <h3 class="text-lg font-semibold mb-4 text-gray-800">🖼️ Immagini della Struttura</h3>
    <div class="mb-6">
        <label for="logo" class="block text-gray-700 font-bold mb-2">Logo Struttura (opzionale)</label>
    ...
    <div class="mb-6">
        ...
        <p class="text-xs text-gray-500 mt-1">Questa sarà l'immagine grande mostrata in cima alla pagina.</p>
    </div>
    <div class="mb-4">
