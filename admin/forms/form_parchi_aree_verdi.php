...
<!-- Basic Info -->
<div class="mb-4">
    <label for="title" class="block text-gray-700 font-bold mb-2">Nome Parco / Area Verde</label>
...
<!-- Custom Fields for Parks -->
<div class="mt-6 p-6 bg-green-50 rounded-lg border border-green-200">
    <h3 class="text-lg font-semibold mb-4 text-green-800">🌳 Dettagli Parco</h3>

    <div class="mb-4">
        <label for="allowed_activities" class="block text-gray-700 font-bold mb-2">Attività Consentite</label>
        <input type="text" name="json_data[allowed_activities]" id="allowed_activities" class="w-full px-3 py-2 border rounded-lg" placeholder="Es. Trekking, Picnic, Birdwatching" value="<?php echo htmlspecialchars($json_data['allowed_activities'] ?? ''); ?>">
    </div>
    <div class="mb-4">
        <label for="trails" class="block text-gray-700 font-bold mb-2">Dettagli Sentieri</label>
        <textarea name="json_data[trails]" id="trails" rows="6" class="w-full px-3 py-2 border rounded-lg font-mono text-sm"><?php echo htmlspecialchars($json_data['trails'] ?? ''); ?></textarea>
        <p class="text-xs text-gray-500 mt-1">Un sentiero per riga. Formato: <strong>Nome Sentiero: Difficoltà: Durata: Lunghezza: Descrizione</strong><br>Es: <strong>Sentiero del Lupo: Media: 2h: 5km: Percorso panoramico con vista sulla valle.</strong></p>
    </div>
</div>
...
<div class="mt-6 p-6 bg-gray-50 rounded-lg border">
    ...
    <?php
        $predefined_services = ['Aree Picnic', 'Punti di Ristoro', 'Parcheggio', 'Bagni Pubblici', 'Centro Visitatori'];
...
<!-- Location -->
<div class="mt-6 p-6 bg-gray-50 rounded-lg border">
    <h3 class="text-lg font-semibold mb-4 text-gray-800">📍 Localizzazione</h3>
     <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div class="mb-4">
            <label for="maps_link" class="block text-gray-700 font-bold mb-2">Link a Mappa del Parco</label>
            <input type="url" name="json_data[maps_link]" id="maps_link" class="w-full px-3 py-2 border rounded-lg" placeholder="https://..." value="<?php echo htmlspecialchars($json_data['maps_link'] ?? ''); ?>">
        </div>
    </div>
</div>
...
<!-- Description -->
<div class="mt-6 p-6 bg-gray-50 rounded-lg border">
    <h3 class="text-lg font-semibold mb-4 text-gray-800">📝 Descrizione (Flora, Fauna, etc.)</h3>
...
<div class="mt-6 p-6 bg-gray-50 rounded-lg border">
    <h3 class="text-lg font-semibold mb-4 text-gray-800">🖼️ Immagini</h3>
     <div class="mb-6">
        <label for="logo" class="block text-gray-700 font-bold mb-2">Logo Parco</label>
