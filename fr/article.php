...
// Load article (translation temporarily disabled)
$article = $db->getArticleBySlug($slug);

if (!$article) {
    header('HTTP/1.0 404 Not Found');
    echo '<div>Article not found</div>';
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_comment'])) {
    ...
    if (empty($author_name) || empty($author_email) || empty($content)) {
        ...
        $commentError = 'You must select a rating from 1 to 5 stars.';
    } else {
        try {
            $stmt = $db->prepare("
            ...
            if ($stmt->execute([$article['id'], $author_name, $author_email, $content, $rating])) {
                $commentMessage = 'Comment submitted successfully!';
                // Clear form data
                $_POST = [];
            } else {
                $commentError = 'Error while submitting comment.';
            }
        } catch (Exception $e) {
        ...
        <article class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-lg">
            ...
            <?php if ($article['featured_image']): ?>
                <div class="relative mb-8">
                    <img src="<?php echo htmlspecialchars(getImageUrl($article['featured_image'])); ?>" alt="<?php echo htmlspecialchars($article['title']); ?>" class="w-full h-auto rounded-lg">
                    
                    <!-- Visit City Button -->
                    <?php if ($article['city_name']): ?>
                    <div class="absolute bottom-4 right-4">
                        <a href="city-detail.php?id=<?php echo $article['city_id'] ?? ''; ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-full font-semibold shadow-lg transition-colors inline-flex items-center">
                            <i data-lucide="map-pin" class="w-4 h-4 mr-2"></i>
                            <span>Visit</span> <?php echo htmlspecialchars($article['city_name']); ?>
                        </a>
                    </div>
                    <?php endif; ?>
                </div>
            ...
            <div class="mt-12 bg-blue-50 p-6 rounded-lg border border-blue-200">
                <h3 class="text-2xl font-bold text-blue-800 mb-6 flex items-center">
                    ...
                    <span>Information about</span> <?php echo htmlspecialchars($article['category_name']); ?>
                </h3>
            ...
            <?php if (!empty($article['gallery_images'])): 
                ...
                <div class="mt-12">
                    <h3 class="text-2xl font-bold text-gray-900 mb-6">Photo Gallery</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <?php foreach ($galleryImages as $index => $imagePath): ?>
                            <div class="relative group cursor-pointer" onclick="openGalleryModal(<?php echo $index; ?>)">
                                <img src="<?php echo htmlspecialchars(getImageUrl($imagePath)); ?>" 
                ...
                <div id="galleryModal" class="fixed inset-0 bg-black bg-opacity-75 hidden z-50 flex items-center justify-center p-4">
                    <div class="relative max-w-4xl max-h-full">
                        ...
                        <button onclick="closeGalleryModal()" class="absolute top-4 right-4 text-white hover:text-gray-300 text-3xl font-bold" title="Close">
                            ×
                        </button>
            ...
            <div class="mt-16 border-t border-gray-200 pt-12">
                <h3 class="text-3xl font-bold text-gray-900 mb-8 flex items-center">
                    ...
                    Comments <span class="text-lg text-gray-500 ml-2">(<?php echo count($comments); ?>)</span>
                </h3>

                <!-- New Comment Form -->
                <div class="bg-gray-50 p-6 rounded-lg mb-8">
                    <h4 class="text-xl font-semibold text-gray-900 mb-4">Leave a Comment</h4>
                    ...
                    <form method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">Click on the stars to give a rating</p>
                        </div>
                        
                        <div>
                            <label for="content" class="block text-sm font-medium text-gray-700 mb-2">Comment *</label>
                            <textarea id="content" name="content" rows="4" required placeholder="Write your comment..."
