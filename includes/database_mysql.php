...
class Database {
    ...
    public function getArticles($limit = null, $offset = 0, $onlyPublished = true) {
        if (!$this->isConnected()) { return []; }
        $sql = 'SELECT a.*, c.name as category_name, p.name as province_name, ci.name as city_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id LEFT JOIN provinces p ON a.province_id = p.id LEFT JOIN cities ci ON a.city_id = ci.id';
        $params = [];
        if ($onlyPublished) {
            $sql .= ' WHERE a.status = ?';
            $params[] = 'published';
    ...
    public function getArticlesByCity($cityId, $limit = null) {
        if (!$this->isConnected()) { return []; }
        $sql = 'SELECT a.*, c.name as category_name, p.name as province_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id LEFT JOIN provinces p ON a.province_id = p.id WHERE a.city_id = ? AND a.status = ? ORDER BY a.created_at DESC';
        if ($limit) {
            $sql .= ' LIMIT ' . (int)$limit;
        }
        $stmt = $this->pdo->prepare($sql);
    ...
    public function getFeaturedArticles($limit = 6) {
        if (!$this->isConnected()) { return []; }
        $stmt = $this->pdo->prepare('SELECT a.*, c.name as category_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id WHERE a.featured = 1 AND a.status = ? ORDER BY a.views DESC LIMIT ?');
    ...
    public function getArticlesByCategory($categoryId, $limit = null) {
        if (!$this->isConnected()) { return []; }
        $sql = 'SELECT a.*, c.name as category_name, p.name as province_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id LEFT JOIN provinces p ON a.province_id = p.id WHERE a.category_id = ? AND a.status = ? ORDER BY a.created_at DESC';
        if ($limit) {
            $sql .= ' LIMIT ' . (int)$limit;
        }
        $stmt = $this->pdo->prepare($sql);
    ...
    public function getArticlesByProvince($provinceId, $limit = null) {
        if (!$this->isConnected()) { return []; }
        $sql = 'SELECT a.*, c.name as category_name, p.name as province_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id LEFT JOIN provinces p ON a.province_id = p.id WHERE a.province_id = ? AND a.status = ? ORDER BY a.created_at DESC';
        if ($limit) {
            $sql .= ' LIMIT ' . (int)$limit;
        }
        $stmt = $this->pdo->prepare($sql);
    ...
    public function getArticleBySlug($slug) {
        if (!$this->isConnected()) { return null; }
        $stmt = $this->pdo->prepare('SELECT a.*, c.name as category_name, p.name as province_name, ci.name as city_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id LEFT JOIN provinces p ON a.province_id = p.id LEFT JOIN cities ci ON a.city_id = ci.id WHERE a.slug = ?');
        $stmt->execute([$slug]);
        return $stmt->fetch();
    }
    ...
    public function searchArticles($query, $provinceId = null) {
        if (!$this->isConnected()) { return []; }
        $sql = 'SELECT a.*, c.name as category_name, p.name as province_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id LEFT JOIN provinces p ON a.province_id = p.id WHERE (a.title LIKE ? OR a.content LIKE ? OR a.excerpt LIKE ?) AND a.status = ?';
    ...
    public function createEventSuggestion($title, $description, $start_date, $end_date, $location, $category_id, $province_id, $organizer, $contact_email, $contact_phone = null, $website = null, $price = 0) {
        if (!$this->isConnected()) { return false; }
        $stmt = $this->pdo->prepare('INSERT INTO events (title, description, start_date, end_date, location, category_id, province_id, organizer, contact_email, contact_phone, website, price, status, source) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "pending", "suggestion")');
    ...
    public function getRevenueForPeriod($startDate, $endDate) {
        ...
        return $subscriptionRevenue + $consumptionRevenue;
    }
