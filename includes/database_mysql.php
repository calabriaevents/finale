...
class Database {
    ...
    public function createArticle($title, $slug, $content, $excerpt, $category_id, $province_id, $city_id, $status, $author = 'Admin', $featured_image = null, $gallery_images = null) {
        if (!$this->isConnected()) { return false; }
        $sql = "INSERT INTO articles (title, slug, content, excerpt, category_id, province_id, city_id, status, author, featured_image, gallery_images, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$title, $slug, $content, $excerpt, $category_id, $province_id, $city_id, $status, $author, $featured_image, $gallery_images]);
        return $this->pdo->lastInsertId();
    }

    public function updateArticle($id, $title, $slug, $content, $excerpt, $category_id, $province_id, $city_id, $status, $featured_image = null, $gallery_images = null) {
        if (!$this->isConnected()) { return 0; }
        $sql = "UPDATE articles SET title = ?, slug = ?, content = ?, excerpt = ?, category_id = ?, province_id = ?, city_id = ?, status = ?, featured_image = ?, gallery_images = ?, updated_at = NOW() WHERE id = ?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$title, $slug, $content, $excerpt, $category_id, $province_id, $city_id, $status, $featured_image, $gallery_images, $id]);
        return $stmt->rowCount();
    }
    ...
    public function getArticleBySlug($slug) {
        if (!$this->isConnected()) { return null; }
        $stmt = $this->pdo->prepare('SELECT a.id, a.title, a.slug, a.content, a.excerpt, a.category_id, a.province_id, a.city_id, a.status, a.author, a.featured_image, a.gallery_images, a.created_at, a.updated_at, a.views, a.featured, c.name as category_name, p.name as province_name, ci.name as city_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id LEFT JOIN provinces p ON a.province_id = p.id LEFT JOIN cities ci ON a.city_id = ci.id WHERE a.slug = ?');
        $stmt->execute([$slug]);
        return $stmt->fetch();
    }
    ...
    public function getUserUploads($status = null, $type = null) {
        ...
        // JOIN con articles per gli articoli e cities per le città
        $sql = 'SELECT u.*, a.title as article_title, c.name as city_name FROM user_uploads u
    ...
    public function getUserUploadById($id) {
        if (!$this->isConnected()) { return null; }
        // JOIN con articles per gli articoli e cities per le città
        $stmt = $this->pdo->prepare('SELECT u.*, a.title as article_title, c.name as city_name FROM user_uploads u
                                    LEFT JOIN articles a ON u.article_id = a.id 
                                    LEFT JOIN cities c ON u.city_id = c.id
                                    WHERE u.id = ?');
        $stmt->execute([$id]);
    ...
    public function createCityPhotoUpload($city_id, $user_name, $user_email, $image_path, $original_filename, $description = null) {
        ...
        $sql = "INSERT INTO user_uploads (city_id, user_name, user_email, image_path, original_filename, description, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
        $stmt = $this->pdo->prepare($sql);
    ...
    public function createPlaceSuggestion($name, $description, $location, $suggested_by_name, $suggested_by_email) {
        if (!$this->isConnected()) { return false; }
        $sql = "INSERT INTO place_suggestions (name, description, address, suggested_by_name, suggested_by_email, status, created_at) VALUES (?, ?, ?, ?, ?, 'pending', NOW())";
        $stmt = $this->pdo->prepare($sql);
