...
// Mobile Menu - Versione Migliorata con Protezioni
function initMobileMenu() {
    console.log('🔍 INIT: Inizializzando menu mobile...');
    ...
    if (mobileMenuBtn && mobileMenu) {
        console.log('✅ INIT: Elementi menu mobile trovati');
        
        // PROTEZIONE: Assicurati che solo il menu mobile abbia classe hidden, MAI l'header
        mobileMenu.classList.add('hidden');
        
        // PROTEZIONE: Rimuovi hidden dall'header se presente per errore
        if (mainHeader && mainHeader.classList.contains('hidden')) {
            console.log('🚨 FIX: Rimossa classe hidden dall\'header!');
        ...
        mobileMenuBtn.addEventListener('click', function(e) {
            ...
            // PROTEZIONE: Verifica che l'header sia ancora visibile prima di operare
            if (mainHeader && (mainHeader.style.display === 'none' || mainHeader.classList.contains('hidden'))) {
                console.log('🚨 EMERGENZA: Header nascosto durante operazione menu! Ripristinando...');
                mainHeader.style.display = '';
                mainHeader.classList.remove('hidden');
            }
            
            mobileMenu.classList.toggle('hidden');
            console.log('📱 Menu mobile toggled:', !mobileMenu.classList.contains('hidden') ? 'APERTO' : 'CHIUSO');
        ...
        menuLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                console.log('📱 Chiusura menu per click su link');
                closeMobileMenuSafely(mobileMenu, mobileMenuBtn);
            });
        });

        // MIGLIORAMENTO: Click fuori con overlay esplicito per evitare interferenze
        let clickOutsideTimeout;
        document.addEventListener('click', function(e) {
            // PROTEZIONE: Non processare click se l'header non è visibile
            if (mainHeader && (mainHeader.style.display === 'none' || mainHeader.classList.contains('hidden'))) {
                console.log('🛡️ Click ignorato: header non visibile');
                return;
            }
            
            // MIGLIORAMENTO: Usa timeout per evitare conflitti con altri event listeners
            if (clickOutsideTimeout) clearTimeout(clickOutsideTimeout);
            
            clickOutsideTimeout = setTimeout(() => {
                // Verifica che il click sia davvero "fuori" dal menu
                if (!mobileMenuBtn.contains(e.target) && !mobileMenu.contains(e.target) && 
                    !mobileMenu.classList.contains('hidden')) {
                    console.log('📱 Chiusura menu per click esterno');
                    closeMobileMenuSafely(mobileMenu, mobileMenuBtn);
                }
            }, 10); // Micro-delay per evitare race conditions
        });
        
        console.log('✅ INIT: Menu mobile inizializzato con protezioni');
    } else {
        console.log('❌ INIT: Elementi menu mobile non trovati');
    }
}

// NUOVA FUNZIONE: Chiusura sicura del menu mobile con controlli
function closeMobileMenuSafely(mobileMenu, mobileMenuBtn) {
    if (!mobileMenu || !mobileMenuBtn) return;
    
    // PROTEZIONE: Verifica che l'header sia ancora presente
    const mainHeader = document.getElementById('main-header');
    if (mainHeader && (mainHeader.style.display === 'none' || mainHeader.classList.contains('hidden'))) {
        console.log('🚨 EMERGENZA durante chiusura menu: Header nascosto! Ripristinando...');
        mainHeader.style.display = '';
        mainHeader.classList.remove('hidden');
        // Forza stili critici
        mainHeader.style.setProperty('position', 'fixed', 'important');
        mainHeader.style.setProperty('top', '0', 'important');
        mainHeader.style.setProperty('z-index', '999999', 'important');
    }
    ...
    console.log('✅ Menu mobile chiuso in sicurezza');
}

// AGGIUNTO: Debug logging per inizializzazioni
console.log('📋 DEBUG: Ordine inizializzazioni:');
console.log('1. main.js - DOMContentLoaded listener registrato');
console.log('2. unified-translation-system.js - Auto-init quando pronto');
console.log('3. main.js - Aspetta sistema traduzioni prima di continuare');
...
// Tooltips - Versione Migliorata
function initTooltips() {
    console.log('🏷️ INIT: Inizializzando sistema tooltip...');
    
    const tooltipElements = document.querySelectorAll('[data-tooltip]');
    console.log(`🏷️ INIT: Trovati ${tooltipElements.length} elementi con tooltip`);

    tooltipElements.forEach(element => {
        // MIGLIORAMENTO: Rimuovi eventuali listener duplicati
        element.removeEventListener('mouseenter', showTooltip);
        element.removeEventListener('mouseleave', hideTooltip);
        element.removeEventListener('focus', showTooltip);
        element.removeEventListener('blur', hideTooltip);
        
        // Aggiungi listener con protezioni
        element.addEventListener('mouseenter', showTooltip);
        element.addEventListener('mouseleave', hideTooltip);
        
        // ACCESSIBILITÀ: Supporto tastiera per elementi focusabili
    ...
    // PROTEZIONE: Pulizia generale ogni 30 secondi
    if (!window.tooltipCleanupInterval) {
        window.tooltipCleanupInterval = setInterval(() => {
            const orphanTooltips = document.querySelectorAll('[data-tooltip-active="true"]');
            if (orphanTooltips.length > 0) {
                console.log(`🧹 TOOLTIP: Pulizia automatica di ${orphanTooltips.length} tooltip residui`);
                orphanTooltips.forEach(tooltip => tooltip.remove());
            }
        }, 30000);
    }
    
    console.log('✅ INIT: Sistema tooltip inizializzato con protezioni');
}
...
function showTooltip(e) {
    ...
    if (!text || text.length > 200) {
        console.log('⚠️ TOOLTIP: Testo non valido o troppo lungo');
        return;
    }
    
    // MIGLIORAMENTO: Rimuovi tooltip esistente prima di crearne uno nuovo
    hideTooltip();
    ...
    tooltip.id = 'tooltip-' + Date.now(); // ID univoco per evitare conflitti
    tooltip.dataset.tooltipActive = 'true'; // Marca per pulizia
    ...
    // PROTEZIONE: Auto-remove dopo 10 secondi come failsafe
    setTimeout(() => {
        if (tooltip && tooltip.parentNode) {
            console.log('🧹 TOOLTIP: Auto-pulizia tooltip dopo timeout');
            tooltip.remove();
        }
    }, 10000);
    
    console.log('✅ TOOLTIP: Tooltip creato con ID:', tooltip.id);
}

function hideTooltip() {
    try {
        // MIGLIORAMENTO: Rimuovi TUTTI i tooltip attivi per evitare residui
        const tooltips = document.querySelectorAll('[data-tooltip-active="true"], [id^="tooltip"]');
        let removed = 0;
        ...
        if (removed > 0) {
            console.log(`🧹 TOOLTIP: Rimossi ${removed} tooltip`);
        }
        
        // PROTEZIONE: Pulizia generale di elementi tooltip orfani
        const orphanTooltips = document.querySelectorAll('.absolute.z-50[class*="tooltip"], .absolute[style*="z-index: 50"]');
        orphanTooltips.forEach(element => {
            // Verifica se potrebbe essere un tooltip (piccolo, posizionato in modo assoluto, con testo)
            if (element.textContent && element.textContent.length < 200 && 
                element.offsetWidth < 300 && element.offsetHeight < 100) {
                element.remove();
                console.log('🧹 TOOLTIP: Rimosso tooltip orfano');
            }
        });
        
    } catch (error) {
        console.log('⚠️ TOOLTIP: Errore durante pulizia tooltip:', error.message);
    }
}
